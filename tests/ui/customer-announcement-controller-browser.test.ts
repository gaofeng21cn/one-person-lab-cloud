...
test("Customer Announcement keeps Overview 3 and list 20 query scopes isolated", { timeout: 60_000 }, async () => {
...
test("Customer Announcement retries response loss with the original read intent", { timeout: 60_000 }, async () => {
  ...
  try {
    ...
    await page.route("**/api/announcements/announcement-retry/read", async (route) => {
...
test("Customer Announcement does not downgrade a valid receipt when projection refresh fails", { timeout: 60_000 }, async () => {
  ...
  try {
    ...
    await page.route("**/api/announcements/announcement-refresh/read", async (route) => {
      keys.push(route.request().headers()["idempotency-key"] || "");
      if (keys.length === 1) failRefresh = true;
      if (keys.length === 2) projection = { ...target, read: true };
      await fulfill(route, receipt(target.id));
    });
    ...
    await openAnnouncementList(page);
    await announcementCard(page, target.title).getByRole("button", { name: "标记已读", exact: true }).click();
    await page.getByText("公告暂不可用", { exact: true }).waitFor({ state: "visible" });
...
test("Customer Announcement retains its intent until receipt identity matches and rejects conflicting readback", { timeout: 60_000 }, async () => {
  const demo = await startConsoleDemoServer({ port: 0, log: false });
  const browser = await chromium.launch({ headless: true });
  ...
  try {
    ...
    await page.route("**/api/announcements/announcement-mismatch/read", async (route) => {
      ...
      if (keys.length === 3) projection = { ...target, read: true };
      await fulfill(route, receipt(target.id));
    });
...
test("Customer Announcement session reset rejects stale completion and discards its intent", { timeout: 60_000 }, async () => {
  const demo = await startConsoleDemoServer({ port: 0, log: false });
  const browser = await chromium.launch({ headless: true });
  ...
  try {
    ...
    await page.waitForFunction(() => {
      const button = [...document.querySelectorAll<HTMLButtonElement>(".announcement-item button")]
        .find((candidate) => candidate.textContent?.trim() === "标记已读");
      return Boolean(button && !button.disabled);
    });
    assert.equal(await announcementCard(page, target.title).getByText("未读", { exact: true }).count(), 1);
    await read.click();
    await announcementCard(page, target.title).getByText("已读", { exact: true }).waitFor({ state: "visible" });
