...
var (
	...
	ErrOperationNotFound = errors.New("tenant operation not found")
	...
	ErrNoCommittedEvidence = errors.New("tenant owner has no committed aggregate evidence")
	// ErrOperationTerminal reports an attempt to change a terminal operation.
	ErrOperationTerminal = errors.New("tenant operation is already terminal")
	// ErrInvalidOperationInput reports a missing or invalid required field.
	ErrInvalidOperationInput = errors.New("invalid tenant operation input")
)

// Operation is the owner-local operation record for the CloudIdentity domain.
// The stage and kind values come from the typed v2.26 contract; this package
// stores them and never invents an alternative vocabulary.
...
func CreateOperation(ctx context.Context, tx *sql.Tx, input OperationInput) (Operation, error) {
	...
	if err != nil {
		return Operation{}, fmt.Errorf("create tenant operation: %w", err)
	}
...
func (s *Store) ReadOperation(ctx context.Context, operationID string) (Operation, error) {
	...
	if s.db == nil {
		...
		return Operation{}, errors.New("tenant store database is required")
	}
	var (
		operation   Operation
	...
	if err != nil {
		return Operation{}, fmt.Errorf("read tenant operation: %w", err)
	}
...
// the operation is already terminal, returns it unchanged. CloudIdentity does
// not invent a success for an unknown external result.
...
func (s *Store) CompleteOperation(ctx context.Context, operationID, status, stage, errorCode, observation string, result json.RawMessage, completedAt time.Time) (Operation, error) {
	...
	if err != nil {
		return Operation{}, fmt.Errorf("complete tenant operation: %w", err)
	}
...
//
// The CloudIdentity owner commits its own tenants, membership, invitations and
// sessions; the path that writes a committed aggregate and its revision belongs
// to that work package. Until it
