// Package store owns the Gateway Integration domain's PostgreSQL schema (`gateway`),
// its migrations, and the records this owner writes.
//
// Gateway Integration is one of the two data owners this deployment unit serves. The
...
// it never shares a pool with the tenant store.
package store
...
// Store reads and writes the Gateway Integration owner's records.
type Store struct {
	db *sql.DB
}
...
func (s *Store) Ready(ctx context.Context) error {
	if s.db == nil {
		return errors.New("gateway store database is required")
	}
...
// guards the `opl_gateway` database. The other owner's block is deliberately not
// applied here.
func EmbeddedMigrations() ([]Migration, error) {
	data, err := migrations.Files.ReadFile(migrations.GatewayFile)
	if err != nil {
		return nil, fmt.Errorf("read embedded migration %s: %w", migrations.GatewayFile, err)
	}
	return []Migration{{
		Version: strings.TrimSuffix(migrations.GatewayFile, ".sql"),
		Query:   string(data),
	}}, nil
}
