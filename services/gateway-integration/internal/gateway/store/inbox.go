...
//
// AggregateType and AggregateID follow the same rule as the Outbox: the
// specification fixes both per exact (eventType, schemaVersion) through
// `x-aggregate-identity`, so an inbound event is validated against that mapping
...
func DeliverInbox(ctx context.Context, tx *sql.Tx, event InboundEvent, receivedAt time.Time) (InboxResult, error) {
	...
	if errors.Is(err, sql.ErrNoRows) {
		...
		return InboxResult{}, fmt.Errorf("read gateway inbox event: %w", err)
	}
	...
	if _, err := tx.ExecContext(ctx, `
		...
		return InboxResult{}, fmt.Errorf("record gateway inbox event: %w", err)
	}
	return InboxResult{Decision: InboxCommit, Applied: true}, nil
}

// MarkInboxProcessed stamps processed_at with the owner-local result. It fails
// closed with the recorded error code; when processing fails the row keeps
// processed_at NULL and the event remains visible as pending.
func MarkInboxProcessed(ctx context.Context, tx *sql.Tx, sourceOwner, sourceEventID, resultResourceID, errorCode string, processedAt time.Time) error {
	...
	if _, err := tx.ExecContext(ctx, `
		...
		return fmt.Errorf("mark gateway inbox event processed: %w", err)
	}
	return nil
}
