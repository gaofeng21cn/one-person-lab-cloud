...
func TestHashRequestBodyIsStableAndContentSensitive(t *testing.T) {
	first := HashRequestBody([]byte(`{"keyBindingId":"key-1"}`))
	second := HashRequestBody([]byte(`{"keyBindingId":"key-1"}`))
	other := HashRequestBody([]byte(`{"keyBindingId":"key-2"}`))
...
func TestSchemaIsTheGatewayOwnerSchema(t *testing.T) {
	if Schema != "gateway" {
		t.Fatalf("Schema = %q, want gateway", Schema)
	}
}
...
func TestEmbeddedMigrationIsThisOwnersVerbatimBlockOnly(t *testing.T) {
	...
	for _, foreign := range []string{"opl_tenant", "CREATE SCHEMA tenant", "CREATE TABLE tenant."} {
		if strings.Contains(query, foreign) {
			t.Fatalf("gateway migration must not contain %q", foreign)
		}
	}
}
