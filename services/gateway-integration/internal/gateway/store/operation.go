...
var (
	...
	ErrInvalidOperationInput = errors.New("invalid gateway operation input")
)

// Operation is the owner-local operation record for the Gateway Integration domain.
...
func CreateOperation(ctx context.Context, tx *sql.Tx, input OperationInput) (Operation, error) {
	...
	if err != nil {
		return Operation{}, fmt.Errorf("create gateway operation: %w", err)
	}
...
func (s *Store) ReadOperation(ctx context.Context, operationID string) (Operation, error) {
	...
	if err != nil {
		return Operation{}, fmt.Errorf("read gateway operation: %w", err)
	}
...
func (s *Store) CompleteOperation(ctx context.Context, operationID, status, stage, errorCode, observation string, result json.RawMessage, completedAt time.Time) (Operation, error) {
	...
	if err != nil {
		return Operation{}, fmt.Errorf("complete gateway operation: %w", err)
	}
