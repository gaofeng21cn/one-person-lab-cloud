...
// ownerRoute names one of the two data owners this deployment unit serves:
// CloudIdentity (`tenant`) and Gateway Integration (`gateway`). The names are the
// v2.26 owner names, so no additional vocabulary is introduced.
type ownerRoute string

const (
	routeTenant  ownerRoute = "tenant"
	routeGateway ownerRoute = "gateway"
)

// routeFromOperationOwner selects the data owner for an owner-scoped
// OwnerOperations request. The two owners share a process, so the requested owner
// must be resolved explicitly. Any owner this unit does not serve belongs to
// another deployment unit and is rejected rather than guessed.
func routeFromOperationOwner(owner v226.OperationOwnerEnum) (ownerRoute, bool) {
	switch owner {
	case v226.OperationOwnerEnum_OPERATION_OWNER_ENUM_TENANT:
		return routeTenant, true
	case v226.OperationOwnerEnum_OPERATION_OWNER_ENUM_GATEWAY:
		return routeGateway, true
	default:
		return "", false
	}
}

// routeFromOwner selects the data owner for an OwnerCommitReadback request using
// the same rule and the same two owners.
func routeFromOwner(owner v226.OwnerEnum) (ownerRoute, bool) {
	switch owner {
	case v226.OwnerEnum_OWNER_ENUM_TENANT:
		return routeTenant, true
	case v226.OwnerEnum_OWNER_ENUM_GATEWAY:
		return routeGateway, true
	default:
		return "", false
	}
