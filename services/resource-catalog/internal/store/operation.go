...
var (
	...
	ErrOperationNotFound = errors.New("resource catalog operation not found")
	...
	ErrNoCommittedEvidence = errors.New("resource catalog owner has no committed aggregate evidence")
	// ErrOperationTerminal reports an attempt to change a terminal operation.
	ErrOperationTerminal = errors.New("resource catalog operation is already terminal")
	// ErrInvalidOperationInput reports a missing or invalid required field.
	ErrInvalidOperationInput = errors.New("invalid resource catalog operation input")
)

// Operation is the owner-local operation record for the resource catalog domain. The
...
func CreateOperation(ctx context.Context, tx *sql.Tx, input OperationInput) (Operation, error) {
	...
	if err != nil {
		return Operation{}, fmt.Errorf("create resource catalog operation: %w", err)
	}
...
func (s *Store) ReadOperation(ctx context.Context, operationID string) (Operation, error) {
	...
	if s.db == nil {
		...
		return Operation{}, errors.New("resource catalog store database is required")
	}
	var (
		operation   Operation
	...
	if err != nil {
		return Operation{}, fmt.Errorf("read resource catalog operation: %w", err)
	}
...
// the operation is already terminal, returns it unchanged. The resource catalog owner does
...
func (s *Store) CompleteOperation(ctx context.Context, operationID, status, stage, errorCode, observation string, result json.RawMessage, completedAt time.Time) (Operation, error) {
	...
	if err != nil {
		return Operation{}, fmt.Errorf("complete resource catalog operation: %w", err)
	}
...
//
// The path that writes a committed aggregate and its revision for an accepted
// resource catalog operation belongs to this domain's command work package and does
// not exist yet. Until it does this returns ErrNoCommittedEvidence, so a caller
// cannot sign a claim or a grant from an empty digest or a fabricated version.
