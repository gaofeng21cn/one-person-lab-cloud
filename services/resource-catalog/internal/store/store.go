// Package store owns the resource catalog domain's PostgreSQL schema
// (`resource_catalog`), its migrations, and the records this owner writes.
//
...
// Store reads and writes the resource catalog owner's records.
type Store struct {
	db *sql.DB
}
...
func (s *Store) Ready(ctx context.Context) error {
	if s.db == nil {
		return errors.New("resource catalog store database is required")
	}
