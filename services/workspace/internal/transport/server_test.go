...
func TestReadRefusesAnotherOwnerWithoutTouchingTheStore(t *testing.T) {
	...
	for name, owner := range map[string]v226.OperationOwnerEnum{
		...
		"another owner": v226.OperationOwnerEnum_OPERATION_OWNER_ENUM_BUILD,
	} {
	...
	if got := status.Code(err); got == codes.InvalidArgument {
		t.Fatalf("the workspace owner must pass addressing, got %v", got)
	}
}
...
// Delivery names one logical inbox, and the transport peer must be the owner the
// envelope claims, so a producer cannot acknowledge into another owner's Inbox or
...
func TestDeliverRefusesAnotherInbox(t *testing.T) {
	...
	for name, owner := range map[string]v226.OwnerEnum{
		...
		"another owner": v226.OwnerEnum_OWNER_ENUM_LEDGER,
	} {
		t.Run(name, func(t *testing.T) {
			_, err := server.Deliver(context.Background(), &v226.DeliverEventRequest{
				Event:                 envelope,
				AuthenticatedProducer: "runtime_control",
				ConsumerOwner:         owner,
			})
			if got := codeOf(t, err); got != codes.InvalidArgument {
				t.Fatalf("Deliver into %s inbox = %v, want InvalidArgument", name, got)
			}
		})
	}
	...
	_, err := server.Deliver(context.Background(), &v226.DeliverEventRequest{
		...
		ConsumerOwner:         v226.OwnerEnum_OWNER_ENUM_WORKSPACE,
	})
