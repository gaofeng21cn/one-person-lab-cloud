...
func TestStageFromTextUsesTheContractVocabulary(t *testing.T) {
	cases := map[string]v226.OperationStageEnum{
		...
		"period_update":      v226.OperationStageEnum_OPERATION_STAGE_ENUM_PERIOD_UPDATE,
		"workspace_deletion": v226.OperationStageEnum_OPERATION_STAGE_ENUM_WORKSPACE_DELETION,
...
func TestKindStatusAndObservationMapping(t *testing.T) {
	if got := kindFromText("create_workspace"); got != v226.OperationKindEnum_OPERATION_KIND_ENUM_CREATE_WORKSPACE {
		t.Fatalf("kindFromText(create_workspace) = %v", got)
	}
...
func TestErrorCodeMappingRejectsUnknownText(t *testing.T) {
	code := errorCodeFromText("workspace_not_ready")
	if code == nil || *code != v226.ErrorCodeEnum_ERROR_CODE_ENUM_WORKSPACE_NOT_READY {
		t.Fatalf("errorCodeFromText(workspace_not_ready) = %v", code)
	}
