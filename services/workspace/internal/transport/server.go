// Package transport exposes the workspace owner's internal typed gRPC surface.
//
...
// Server implements the workspace owner's internal gRPC services over its own store.
type Server struct {
...
// NewServer returns the workspace owner's internal server.
func NewServer(workspaceStore *store.Store) *Server {
	return &Server{store: workspaceStore}
}
...
func (s *Server) ReadOwnerCommit(ctx context.Context, request *v226.ReadOwnerCommitRequest) (*v226.OwnerCommitEvidence, error) {
	if request == nil || request.GetOwner() != v226.OwnerEnum_OWNER_ENUM_WORKSPACE {
...
func (s *Server) Deliver(ctx context.Context, request *v226.DeliverEventRequest) (*v226.InboxAck, error) {
	...
	return nil, status.Error(codes.Unimplemented, "workspace inbox dispatch is implemented with the workspace domain owner")
}

// ownerName is this owner's logical name in the v2.26 domain set.
const ownerName = "workspace"
...
func requireOperationOwner(owner v226.OperationOwnerEnum) error {
	if owner != v226.OperationOwnerEnum_OPERATION_OWNER_ENUM_WORKSPACE {
...
func requireConsumerOwner(owner v226.OwnerEnum) error {
	if owner != v226.OwnerEnum_OWNER_ENUM_WORKSPACE {
		return status.Errorf(codes.InvalidArgument, "this endpoint serves owner %s", ownerName)
	}
	return nil
}
...
func toProtoOperation(operation store.Operation) *v226.Operation {
	return &v226.Operation{
		...
		Owner:             v226.OperationOwnerEnum_OPERATION_OWNER_ENUM_WORKSPACE,
