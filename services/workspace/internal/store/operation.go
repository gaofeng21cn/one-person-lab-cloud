...
var (
	...
	ErrOperationNotFound = errors.New("workspace operation not found")
	...
	ErrNoCommittedEvidence = errors.New("workspace owner has no committed aggregate evidence")
	// ErrOperationTerminal reports an attempt to change a terminal operation.
	ErrOperationTerminal = errors.New("workspace operation is already terminal")
	// ErrInvalidOperationInput reports a missing or invalid required field.
	ErrInvalidOperationInput = errors.New("invalid workspace operation input")
)

// Operation is the owner-local operation record for the workspace domain. The
...
func CreateOperation(ctx context.Context, tx *sql.Tx, input OperationInput) (Operation, error) {
	...
	if err != nil {
		return Operation{}, fmt.Errorf("create workspace operation: %w", err)
	}
...
func (s *Store) ReadOperation(ctx context.Context, operationID string) (Operation, error) {
	...
	if s.db == nil {
		...
		return Operation{}, errors.New("workspace store database is required")
	}
	var (
		operation   Operation
	...
	if err != nil {
		return Operation{}, fmt.Errorf("read workspace operation: %w", err)
	}
...
// the operation is already terminal, returns it unchanged. The workspace owner does
...
func (s *Store) CompleteOperation(ctx context.Context, operationID, status, stage, errorCode, observation string, result json.RawMessage, completedAt time.Time) (Operation, error) {
	...
	if err != nil {
		return Operation{}, fmt.Errorf("complete workspace operation: %w", err)
	}
...
//
// The workspace domain commits its own subscription, period-obligation, plan-change
// and workspace aggregates; the path that writes a committed aggregate and its
// revision belongs to that work package. Until it exists this returns
// ErrNoCommittedEvidence, so a caller cannot sign a claim or a grant from an empty
