...
func TestDecideInboxComparesTheWholeImmutableIdentity(t *testing.T) {
	...
	for name, mutate := range map[string]func(*RecordedIdentity){
		"event type":         func(r *RecordedIdentity) { r.EventType = "tenant.reenabled.v1" },
		"schema version":     func(r *RecordedIdentity) { r.SchemaVersion = 2 },
		"aggregate type":     func(r *RecordedIdentity) { r.AggregateType = "workspace" },
		"aggregate id":       func(r *RecordedIdentity) { r.AggregateID = "rt-2" },
...
func TestEventValidationRejectsIncompleteInputAndHashesPayload(t *testing.T) {
	valid := Event{
		...
		AggregateRevision: 1,
		CorrelationID:     "req-1",
		Payload:           json.RawMessage(`{"workspaceId":"ws-1"}`),
	...
	for name, mutate := range map[string]func(*Event){
		...
		"invented event version":      func(e *Event) { e.EventType = "workspace.quote_accepted.v1" },
		...
		"payload without the id":      func(e *Event) { e.Payload = json.RawMessage(`{"subscriptionId":"sub-1"}`) },
	} {
		t.Run(name, func(t *testing.T) {
			candidate := valid
			mutate(&candidate)
...
func TestInboundEventValidationAndHashing(t *testing.T) {
	...
	for name, mutate := range map[string]func(*InboundEvent){
		...
		"invented event version":  func(e *InboundEvent) { e.EventType = "workspace.quote_accepted.v1" },
...
func TestHashRequestBodyIsStableAndContentSensitive(t *testing.T) {
	first := HashRequestBody([]byte(`{"computePlanId":"cp-1"}`))
	second := HashRequestBody([]byte(`{"computePlanId":"cp-1"}`))
	other := HashRequestBody([]byte(`{"computePlanId":"cp-2"}`))
...
func TestEmbeddedMigrationIsTheVerbatimOwnerBlock(t *testing.T) {
	...
	for _, marker := range []string{
		"BEGIN;",
		"current_database() <> 'opl_workspace'",
