...
var (
	...
	ErrOperationNotFound = errors.New("build operation not found")
	...
	ErrNoCommittedEvidence = errors.New("build owner has no committed aggregate evidence")
	// ErrOperationTerminal reports an attempt to change a terminal operation.
	ErrOperationTerminal = errors.New("build operation is already terminal")
	// ErrInvalidOperationInput reports a missing or invalid required field.
	ErrInvalidOperationInput = errors.New("invalid build operation input")
)

// Operation is the owner-local operation record for the build domain. The
...
func CreateOperation(ctx context.Context, tx *sql.Tx, input OperationInput) (Operation, error) {
	...
	if err != nil {
		return Operation{}, fmt.Errorf("create build operation: %w", err)
	}
...
func (s *Store) ReadOperation(ctx context.Context, operationID string) (Operation, error) {
	...
	if s.db == nil {
		...
		return Operation{}, errors.New("build store database is required")
	}
	var (
		operation   Operation
	...
	if err != nil {
		return Operation{}, fmt.Errorf("read build operation: %w", err)
	}
...
// the operation is already terminal, returns it unchanged. The build owner does
...
func (s *Store) CompleteOperation(ctx context.Context, operationID, status, stage, errorCode, observation string, result json.RawMessage, completedAt time.Time) (Operation, error) {
	...
	if err != nil {
		return Operation{}, fmt.Errorf("complete build operation: %w", err)
	}
...
//
// The build domain commits build jobs; the path that writes a committed aggregate
// and its revision belongs to that work package. Until it exists this returns
