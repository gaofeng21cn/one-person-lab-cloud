// Package store owns the build domain's PostgreSQL schema (`build`), its
...
// Store reads and writes the build owner's records.
type Store struct {
	db *sql.DB
}
...
func (s *Store) Ready(ctx context.Context) error {
	if s.db == nil {
		return errors.New("build store database is required")
	}
