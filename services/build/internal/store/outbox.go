...
func AppendEvent(ctx context.Context, tx *sql.Tx, event Event, consumers []string) error {
	...
	if _, err := tx.ExecContext(ctx, `
		...
		return fmt.Errorf("append build outbox event: %w", err)
	}
	for _, consumer := range consumers {
		consumer = strings.TrimSpace(consumer)
		if consumer == "" {
		...
		if _, err := tx.ExecContext(ctx, `
			...
			return fmt.Errorf("append build outbox delivery: %w", err)
		}
	}
	return nil
}
...
func (s *Store) PendingDeliveries(ctx context.Context, consumer string, limit int) ([]PendingDelivery, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("list build pending deliveries: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&delivery.DeliveryID, &delivery.EventID, &delivery.AttemptCount,
			...
			return nil, fmt.Errorf("scan build pending delivery: %w", err)
		}
...
func (s *Store) AcknowledgeDelivery(ctx context.Context, consumer, eventID string) error {
	...
	if _, err := s.db.ExecContext(ctx, `
		...
		return fmt.Errorf("acknowledge build outbox delivery: %w", err)
	}
	return nil
}
