// Package transport exposes the build owner's internal typed gRPC surface.
//
...
// Server implements the build owner's internal gRPC services over its own store.
type Server struct {
...
// NewServer returns the build owner's internal server.
func NewServer(buildStore *store.Store) *Server {
...
func (s *Server) ReadOwnerCommit(ctx context.Context, request *v226.ReadOwnerCommitRequest) (*v226.OwnerCommitEvidence, error) {
	...
	if request.GetOwner() != v226.OwnerEnum_OWNER_ENUM_BUILD {
