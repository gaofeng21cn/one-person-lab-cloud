...
var (
	...
	ErrOperationNotFound = errors.New("runtime control operation not found")
	...
	ErrNoCommittedEvidence = errors.New("runtime control owner has no committed aggregate evidence")
	// ErrOperationTerminal reports an attempt to change a terminal operation.
	ErrOperationTerminal = errors.New("runtime control operation is already terminal")
	// ErrInvalidOperationInput reports a missing or invalid required field.
	ErrInvalidOperationInput = errors.New("invalid runtime control operation input")
)

// Operation is the owner-local operation record for the runtime control domain. The
...
func CreateOperation(ctx context.Context, tx *sql.Tx, input OperationInput) (Operation, error) {
	...
	if err != nil {
		return Operation{}, fmt.Errorf("create runtime control operation: %w", err)
	}
...
func (s *Store) ReadOperation(ctx context.Context, operationID string) (Operation, error) {
	...
	if s.db == nil {
		...
		return Operation{}, errors.New("runtime control store database is required")
	}
	var (
		operation   Operation
	...
	if err != nil {
		return Operation{}, fmt.Errorf("read runtime control operation: %w", err)
	}
...
// the operation is already terminal, returns it unchanged. The runtime control owner does
...
func (s *Store) CompleteOperation(ctx context.Context, operationID, status, stage, errorCode, observation string, result json.RawMessage, completedAt time.Time) (Operation, error) {
	...
	if err != nil {
		return Operation{}, fmt.Errorf("complete runtime control operation: %w", err)
	}
...
//
// This owner commits its runtime instances and runtime actions; the path that
// writes a committed aggregate revision belongs to that work package. Until it
