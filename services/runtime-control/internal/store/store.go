// Package store owns the runtime control domain's PostgreSQL schema (`runtime_control`), its
...
// Store reads and writes the runtime control owner's records.
type Store struct {
	db *sql.DB
}
...
func (s *Store) Ready(ctx context.Context) error {
	if s.db == nil {
		return errors.New("runtime control store database is required")
	}
