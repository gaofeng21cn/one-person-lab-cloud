...
// Operation status values. The database CHECK constraint enforces the same set;
// this is the only vocabulary this package stores.
const (
...
// Observation results distinguish an unknown external outcome from a failure. An
// unknown result is never treated as success.
const (
...
var (
	...
	ErrOperationNotFound = errors.New("operation not found in this owner")
	...
	ErrNoCommittedEvidence = errors.New("this owner has no committed aggregate evidence")
...
// Operation is the owner-local operation record. Kind and Stage come from the
// typed contract; this package stores them and invents no alternative vocabulary.
...
// CreateOperation records an operation. It runs in the caller's transaction,
// which also carries the durable command and any Outbox event.
func (s *Store) CreateOperation(ctx context.Context, tx *sql.Tx, input OperationInput) (Operation, error) {
	...
	err := tx.QueryRowContext(ctx, `
		INSERT INTO `+s.schema+`.operations
	...
	if err != nil {
		return Operation{}, fmt.Errorf("create %s operation: %w", s.schema, err)
	}
	return operation, nil
}

// ReadOperation loads one operation owned by this service.
func (s *Store) ReadOperation(ctx context.Context, operationID string) (Operation, error) {
	...
	if s.db == nil {
		return Operation{}, errors.New("ownerstore: database is required")
	}
	var (
		operation   Operation
	...
	err := s.db.QueryRowContext(ctx, `
		...
		FROM `+s.schema+`.operations WHERE id = $1`, operationID).
		Scan(&operation.ID, &operation.TenantID, &operation.ActorID, &operation.Kind,
	...
	if err != nil {
		return Operation{}, fmt.Errorf("read %s operation: %w", s.schema, err)
	}
...
// ReconcileOperation re-reads this owner's own state for an operation. A terminal
// operation is returned unchanged, and the owner never invents a success for an
...
// StartOperation moves an accepted operation to running and records its stage.
func (s *Store) StartOperation(ctx context.Context, operationID, stage string) (Operation, error) {
	if strings.TrimSpace(stage) == "" {
		return Operation{}, fmt.Errorf("%w: stage is required", ErrInvalidOperationInput)
	}
	tag, err := s.db.ExecContext(ctx, `
		UPDATE `+s.schema+`.operations
		SET status = $2, stage = $3, started_at = COALESCE(started_at, now()), updated_at = now()
		WHERE id = $1 AND status NOT IN ('succeeded','failed','cancelled')`,
		operationID, OperationRunning, stage)
	if err != nil {
		return Operation{}, fmt.Errorf("start %s operation: %w", s.schema, err)
	}
...
func (s *Store) CompleteOperation(ctx context.Context, operationID, status, stage, errorCode, observation string, result json.RawMessage, completedAt time.Time) (Operation, error) {
	...
	if err != nil {
		return Operation{}, fmt.Errorf("complete %s operation: %w", s.schema, err)
	}
	rows, err := tag.RowsAffected()
	if err != nil {
		return Operation{}, fmt.Errorf("complete %s operation: %w", s.schema, err)
...
// AdvanceOperationStage records progress on a non-terminal operation without
// changing its status. Progress is never used to claim completion.
func (s *Store) AdvanceOperationStage(ctx context.Context, operationID, stage string, observation string) (Operation, error) {
	if strings.TrimSpace(stage) == "" {
		return Operation{}, fmt.Errorf("%w: stage is required", ErrInvalidOperationInput)
	}
	var nullObservation any
	...
	tag, err := s.db.ExecContext(ctx, `
		UPDATE `+s.schema+`.operations
		SET stage = $2, observation_result = COALESCE($3, observation_result), updated_at = now()
		WHERE id = $1 AND status NOT IN ('succeeded','failed','cancelled')`,
		operationID, stage, nullObservation)
	if err != nil {
		return Operation{}, fmt.Errorf("advance %s operation: %w", s.schema, err)
	}
	rows, err := tag.RowsAffected()
	if err != nil {
		return Operation{}, fmt.Errorf("advance %s operation: %w", s.schema, err)
