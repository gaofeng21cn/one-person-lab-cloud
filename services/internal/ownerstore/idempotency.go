...
// tenant_scope + actor_scope + the stable API operation name + key; it is not the
// per-request Operation entity id.
...
// HashRequestBody returns the hex sha256 of a normalized request body. The caller
// normalizes first; identical bytes must hash identically so a replayed command
...
func (s *Store) LookupIdempotency(ctx context.Context, tx *sql.Tx, input IdempotencyInput) (IdempotencyRecord, bool, error) {
	var (
	...
	err := tx.QueryRowContext(ctx, `
		...
		FROM `+s.schema+`.idempotency_records
	...
	if err != nil {
		return IdempotencyRecord{}, false, fmt.Errorf("read %s idempotency record: %w", s.schema, err)
	}
...
func (s *Store) RecordIdempotency(ctx context.Context, tx *sql.Tx, input IdempotencyInput) error {
	...
	if _, err := tx.ExecContext(ctx, `
		INSERT INTO `+s.schema+`.idempotency_records
		...
		return fmt.Errorf("record %s idempotency: %w", s.schema, err)
	}
	return nil
}
