...
export async function productionReadiness({ env = process.env, commandExists = (command) => commandExistsInPath(command, env) } = {}) {
  ...
  const checks = [
    ...
    check("application_domain", looksLikeProductionDomain(env.OPL_APPLICATION_DOMAIN) && env.OPL_APPLICATION_DOMAIN !== env.OPL_WORKSPACE_DOMAIN, "OPL_APPLICATION_DOMAIN must be a production domain distinct from OPL_WORKSPACE_DOMAIN"),
    check("application_load_balancer", /^lb-[a-z0-9]+$/.test(env.OPL_APPLICATION_INGRESS_EXISTING_LB_ID || ""), "OPL_APPLICATION_INGRESS_EXISTING_LB_ID must be the installation's existing CLB id"),
    check("application_tls_secret", /^[a-z0-9][a-z0-9.-]{0,252}$/.test(env.OPL_APPLICATION_INGRESS_TLS_SECRET || ""), "OPL_APPLICATION_INGRESS_TLS_SECRET must name the secret covering the application origin"),
