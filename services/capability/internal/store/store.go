// Package store owns the capability domain's PostgreSQL schema (`capability`),
...
// Store reads and writes the capability owner's records.
type Store struct {
	db *sql.DB
}
...
func (s *Store) Ready(ctx context.Context) error {
	if s.db == nil {
		return errors.New("capability store database is required")
	}
