...
var (
	...
	ErrNoCommittedEvidence = errors.New("capability owner has no committed aggregate evidence")
...
func (s *Store) ReadOperation(ctx context.Context, operationID string) (Operation, error) {
	...
	if s.db == nil {
		...
		return Operation{}, errors.New("capability store database is required")
	}
	var (
		operation   Operation
...
//
// The capability domain commits packages and versions; the path that writes a
// committed aggregate and its revision belongs to that work package. Until it
