...
文献连接器是 OPL Connect 的优先落地点。早期原型可以继续从 MAS、ScholarSkills 或其他领域 skill 快速验证；当 PubMed、数据库、工具库或大型 skill pack 成为高频路径后，应沉淀为 Fabric 中稳定维护的连接能力。MAS、OPL App 和 OPL Workspace 可以直接调用统一连接器，领域 skill 继续负责检索策略、证据判断、质量下限和写作口径。

## Skill-first 协同

OPL Cloud 支持 skill-first 的能力演进路径：领域 owner 维护主 skill，增强包提供 references、packs 和 quality floors，OPL Connect 负责发现、同步、安装和稳定连接，OPL Ledger 记录 receipt 与 provenance。

```text
领域 skill 原型
→ 高频连接能力
→ OPL Connect / OPL Fabric
→ MAS / Workspace / App 调用
→ OPL Ledger 回执与来源记录
```
