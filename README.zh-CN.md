...
它把 OPL App 和 OPL Workspace 视为等价的用户工作面：OPL App 是本机工作台，OPL Workspace 是云端 Docker/WebUI 工作台。两者都可以使用 OPL Gateway、OPL Fabric 和 OPL Ledger 能力。OPL Fabric 和 OPL Ledger 是可复用的平台能力，不绑定某一个前台入口；OPL Console 负责把托管资源和组织资源纳入权限、生命周期、审计和计费治理。
...
Console 是组织治理台，负责 OPL Cloud 托管或组织纳管的账号、账单、权限、工作空间生命周期、连接器审批、环境策略和资源额度。用户自己提供的本机、SSH、高性能计算或文献连接能力，也可以使用标准 Fabric 流程；当这些资源被组织纳入统一策略时，再进入 Console 管理。

**OPL Fabric 负责资源底座**<br/>
Fabric 承载计算池、存储库、环境目录、连接器注册表和执行适配器。OPL Connect 是 Fabric 中的连接能力，负责把 PubMed 等文献库、数据库、工具和机构系统稳定接入 OPL 工作流。普通用户看到的是“文献检索”“标准计算”“GPU 加速”“私有数据桶”“机构高性能计算”等产品化选项。
...
文献连接器是 OPL Connect 的优先落地点。早期原型可以继续由 ScholarSkills 或 MAS 技能快速验证；当 PubMed 这类能力成为高频路径后，应沉淀为 Fabric 中稳定维护的连接器。MAS、OPL App 和 OPL Workspace 调用统一连接器，领域技能继续负责检索策略、证据判断和写作口径。
