...
for wid,w in W.items():
 ...
 for path in w['plannedWritePaths']:
  ...
  require(any(Path(path).is_relative_to(Path(root)) for root in plan['sourceRoots'].values()),wid+' write escapes declared roots: '+path)
