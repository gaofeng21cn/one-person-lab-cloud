...
| F / `PB-F-TYPED-OPERATIONS-01` | `active` | `P1` | Each Control Plane/Fabric bounded context owns its reducer; live changed paths still contain generic maps/strings and retired Acceptance B surfaces. Fabric callers use capability-specific persistence/provider ports; Workspace Launch Stages and non-secret Runtime status reads have dependency-narrow Engine owners. Control Plane `WorkspaceLaunchReconciler` types Stage, Launch status, and Stage observation state behind its strict schema-v3 codec. Console Workspace Launch, access Secrets, Delete, Renewal, Gateway Budget, Gateway Usage, Gateway Account Read, Customer Workspace Read, Fabric Runtime Read, Billing/Receipt, Wallet Adjustment, Support Mapping, Operator Account, Operator Announcement Lifecycle, Customer Announcement Read, and Operator Resource Read now own complete browser lifecycles behind typed controllers. The broad root retains Session/Router/toast/shell composition, Workspace Budget dependency coordination, and the single-request Operator Overview, Reconciliation, and Health projections; the Console stop-audit found no duplicated owner projection or independent lifecycle left in root | Continue PB-F on the next live Control Plane/Fabric generic changed path. Reopen Console decomposition only when an observed lifecycle, pagination, selection, timer, stale-write risk, or new direct owner read appears; do not split single-request projections merely to shrink the root file, and do not move Session until every child reset contract is explicit | Instance readback must prove no configured or non-terminal Acceptance B consumer before Cloud removes that path |
...
| J / `PB-J-OPERATIONS-RECOVERY-01` | `active` | `P0` | Control Plane Operations and Recovery; exact Tencent/TKE Runtime-ready and first Compute-ownership Launch reviews now converge through system-owned original-operation authorizations, while Registration/Delete/Reset recovery and unified closure evidence are incomplete | Every supported manual-review operation appears once with owner, blocker and allowed action; recovery is idempotent, owner-read, audited, and cannot write another owner's state | Protected Instance operator executes only Cloud-authorized commands and retains receipts |
...
| N / `PB-N-DISTRIBUTION-RELEASE-01` | `verify` | `P0` | Cloud distribution owner; source now admits the exact Candidate, Local qualification, Instance decision and `workspace_verified` evidence, then promotes that digest without rebuild. Managed TKE also requires an approved immutable Workspace image catalog and Cloud keeps new-Launch activation separate from existing-Workspace replacement, but no hosted cohort has proved the full path | Clean install validates the Workspace image catalog/domain/Profile; Console activates one catalog release for new Launches and applies it to an existing Workspace through the audited replacement API; publication promotes the exact Candidate digest with matching assets/checksums and same-tag recovery | Instance injects the protected catalog, deploys the exact Candidate, proves new Launch plus existing-Workspace update/rollback by owner readback, and retains deployment and rollback receipts |
