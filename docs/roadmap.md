...
| F / `PB-F-TYPED-OPERATIONS-01` | `active` | `P1` | Each Control Plane/Fabric bounded context owns its reducer; live changed paths still contain generic maps/strings and retired Acceptance B surfaces. Fabric callers use capability-specific persistence/provider ports; Workspace Launch Stages and non-secret Runtime status reads have dependency-narrow Engine owners. Control Plane `WorkspaceLaunchReconciler` types Stage, Launch status, and Stage observation state behind its strict schema-v3 codec. Console Workspace Launch, Workspace access Secrets, operator Wallet Adjustment, and Support Mapping now own their complete browser lifecycles behind typed controllers; the broad root retains Session/Router, shared reads, and composition. A current broad Console Owner inventory records the remaining candidate capabilities, their readback proofs, reset boundaries, and real consumers. Workspace lifecycle, Gateway budget/usage, operator provisioning/accounts, announcements, route read loading, and ConsoleShell state remain open | Select the next complete capability from the inventory, define its narrow typed contract, switch every real consumer, and delete that capability's root state, intent, timer/generation, readback, reset and command. Wallet Adjustment and Support Mapping are complete with focused intent/recovery and typed readback tests; do not move Session until capability reset contracts are explicit | Instance readback must prove no configured or non-terminal Acceptance B consumer before Cloud removes that path |
