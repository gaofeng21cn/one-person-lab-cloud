...
| F / `PB-F-TYPED-OPERATIONS-01` | `active` | `P1` | Each Control Plane/Fabric bounded context owns its reducer; live changed paths still contain generic maps/strings and retired Acceptance B surfaces. Fabric callers use capability-specific persistence/provider ports; Workspace Launch Stages and non-secret Runtime status reads have dependency-narrow Engine owners. Control Plane `WorkspaceLaunchReconciler` types Stage, Launch status, and Stage observation state behind its strict schema-v3 codec. Console Workspace Launch and Workspace access Secrets now own their complete browser lifecycles behind `WorkspaceLaunchController` and `WorkspaceSecretController`; a current broad Console Owner inventory now records the remaining candidate capabilities, their readback proofs, reset boundaries, and real consumers. The broad root retains Session/Router, shared reads, and composition. Generic serialization boundaries and retired Acceptance B surfaces remain open | Select the next complete capability from the inventory, define its narrow typed contract, switch every real consumer, and delete that capability's root state, intent, timer/generation, readback, reset and command. Do not move Session until capability reset contracts are explicit | Instance readback must prove no configured or non-terminal Acceptance B consumer before Cloud removes that path |
