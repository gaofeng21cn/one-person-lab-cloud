...
| F / `PB-F-TYPED-OPERATIONS-01` | `active` | `P1` | Each Control Plane/Fabric bounded context owns its reducer; live changed paths still contain generic maps/strings and retired Acceptance B surfaces. Fabric callers use capability-specific persistence/provider ports; Workspace Launch Stages and non-secret Runtime status reads have dependency-narrow Engine owners. Control Plane `WorkspaceLaunchReconciler` types Stage, Launch status, and Stage observation state behind its strict schema-v3 codec. Console Workspace Launch, access Secrets, Delete, Renewal, Gateway Budget, Wallet Adjustment, and Support Mapping now own complete browser lifecycles behind typed controllers; the broad root retains Session/Router, shared reads, route composition, and unrelated operator commands. Billing/Receipt, Gateway Usage, operator provisioning/accounts, announcements, route read loading, and ConsoleShell state remain open | Select the next complete capability from the inventory, define its narrow typed contract, switch every real consumer, and delete that capability's root state, intent, timer/generation, readback, reset and command. Workspace Delete, Renewal, and Budget are complete with focused intent, identity, response-loss, owner-readback, cross-navigation retry, and bidirectional stale-projection browser tests; do not move Session until capability reset contracts are explicit | Instance readback must prove no configured or non-terminal Acceptance B consumer before Cloud removes that path |
