...
| F / `PB-F-TYPED-OPERATIONS-01` | `active` | `P1` | Each Control Plane/Fabric bounded context owns its reducer; live changed paths still contain generic maps/strings and retired Acceptance B surfaces. Fabric callers use capability-specific persistence/provider ports; Workspace Launch Stages and non-secret Runtime status reads now have dependency-narrow Engine owners. The next slice is the Control Plane `WorkspaceLaunchReconciler`, whose schema-v3 durable cursor still crosses raw JSON/map fact accessors; Console Controller separation remains open | First type the Reconciler stage/status/observation boundary, reject illegal transitions in focused decoder/reducer tests, preserve schema-v3 data and current HTTP behavior, then switch consumers before removing the retired generic path | Instance readback must prove no configured or non-terminal Acceptance B consumer before Cloud removes that path |
