...
| 单用例部署（镜像＋规格） | 部署命令直接接受镜像与运行规格，不再要求先单独登记一次全局应用版本；revision 快照仍只有唯一 owner。 | 2026-09-17 Cloud 侧完成内联 `revision`：同一命令写入既有准入表并复用同一部署链；冲突内容拒绝且不改写已存 revision。2026-09-20 完成 Console 收敛（阶段 5）与描述补全：管理员只选择 命名空间 → repository → tag/版本，Control Plane 从镜像仓库（同一批准目录）派生应用身份、按描述内容派生版本，并读取镜像自身声明的端口、数据路径与进程身份；未批准镜像、不可读仓库或无法确定入口的发布均按名拒绝，自带身份的发布者描述按原文使用、不被补全。取证见 [描述补全与单命令部署](status.md#deployment-description-completion-local-development)。剩余：Instance 无部署读回。 |
