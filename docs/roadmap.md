...
| F / `PB-F-TYPED-OPERATIONS-01` | `active` | `P1` | Each Control Plane/Fabric bounded context owns its reducer; live changed paths still contain generic maps/strings and retired Acceptance B surfaces. Fabric callers use capability-specific persistence/provider ports; Workspace Launch Stages and non-secret Runtime status reads have dependency-narrow Engine owners. Control Plane `WorkspaceLaunchReconciler` types Stage, Launch status, and Stage observation state behind its strict schema-v3 codec. Console Workspace Launch now owns its complete browser lifecycle behind `WorkspaceLaunchController`, while the broad root retains Session/Router composition. Other broad Console capabilities, generic serialization boundaries, and retired Acceptance B surfaces remain open | Next extract one complete Console capability with an already distinct lifecycle and failure model, starting with the 60-second Secret lifecycle; preserve behavior, switch real consumers, and remove the retired broad path before selecting another slice | Instance readback must prove no configured or non-terminal Acceptance B consumer before Cloud removes that path |
