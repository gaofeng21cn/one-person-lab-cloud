...
| F / `PB-F-TYPED-OPERATIONS-01` | `active` | `P1` | Each Control Plane/Fabric bounded context owns its reducer; live changed paths still contain generic maps/strings and retired Acceptance B surfaces. Fabric callers use capability-specific persistence/provider ports; Workspace Launch Stages and non-secret Runtime status reads have dependency-narrow Engine owners. Control Plane `WorkspaceLaunchReconciler` now types Stage, Launch status, and Stage observation state behind its strict schema-v3 codec, while generic persistence rows and heterogeneous canonical Stage facts remain intentional serialization boundaries. Console Controller separation and retired Acceptance B surfaces remain open | Next establish the Console Controller capability boundary, preserve current Control Plane API and Console behavior with focused tests, switch current consumers, and only then remove the broad controller or retired generic paths | Instance readback must prove no configured or non-terminal Acceptance B consumer before Cloud removes that path |
