...
| F / `PB-F-TYPED-OPERATIONS-01` | `active` | `P1` | Each Control Plane/Fabric bounded context owns its reducer; live changed paths still contain generic maps/strings and retired Acceptance B surfaces. Fabric callers use capability-specific persistence/provider ports; Workspace Launch Stages and non-secret Runtime status reads have dependency-narrow Engine owners. Control Plane `WorkspaceLaunchReconciler` types Stage, Launch status, and Stage observation state behind its strict schema-v3 codec. Console Workspace Launch, access Secrets, Delete, Renewal, Gateway Budget, Gateway Usage, Billing/Receipt, Wallet Adjustment, Support Mapping, Operator Account, Operator Announcement Lifecycle, Customer Announcement Read, and Operator Resource Read now own complete browser lifecycles behind typed controllers; the broad root retains Session/Router, shared reads, route composition, route read loading, and ConsoleShell state | Select the next complete capability from the inventory, define its narrow typed contract, switch every real consumer, and delete that capability's root state, intent, timer/generation, readback, reset and command. Customer Announcement Read is complete with fixed 3/20 route scopes, late-query rejection, per-announcement response-loss key reuse, typed receipt authority, bounded projection refresh, route exit/re-entry, and Session reset tests. Operator Resource Read is complete with fixed 20-item paging, list/detail/policy/preview identity scopes, independent failure settlement, late page/Workspace rejection, route/Session reset, and selected-Workspace replacement refresh tests. Customer read receipts remain independent from Operator Announcement Lifecycle; do not move Session until every child reset contract is explicit | Instance readback must prove no configured or non-terminal Acceptance B consumer before Cloud removes that path |
