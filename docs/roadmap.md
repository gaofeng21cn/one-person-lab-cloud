...
| F / `PB-F-TYPED-OPERATIONS-01` | `active` | `P1` | Each Control Plane/Fabric bounded context owns its reducer; live changed paths still contain generic maps/strings and retired Acceptance B surfaces. Fabric callers use capability-specific persistence/provider ports; Workspace Launch Stages and non-secret Runtime status reads have dependency-narrow Engine owners. Control Plane `WorkspaceLaunchReconciler` types Stage, Launch status, and Stage observation state behind its strict schema-v3 codec. Console Workspace Launch, access Secrets, Delete, Renewal, Gateway Budget, Gateway Usage, Billing/Receipt, Wallet Adjustment, and Support Mapping now own complete browser lifecycles behind typed controllers; the broad root retains Session/Router, shared reads, route composition, and unrelated operator commands. Operator Account, Announcement Lifecycle, route read loading, and ConsoleShell state remain open | Select the next complete capability from the inventory, define its narrow typed contract, switch every real consumer, and delete that capability's root state, intent, timer/generation, readback, reset and command. Workspace Delete, Renewal, and Budget are complete with focused intent, identity, response-loss, owner-readback, cross-navigation retry, and bidirectional stale-projection browser tests; Gateway Usage is complete with Key/period freshness, empty-Key clearing, and independent Usage/Summary failure regressions; Billing/Receipt is extracted with independent list/detail freshness, opaque cursor ownership, and distinct Overview/Billing query limits. Do not move Session until capability reset contracts are explicit | Instance readback must prove no configured or non-terminal Acceptance B consumer before Cloud removes that path |
