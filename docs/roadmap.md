...
| `D4-WORKSPACE-LIFECYCLE-01` | `cloud_complete` | Control Plane lifecycle; Fabric, Sub2API and Console | Original-period renewal, unpaid Runtime suspension, explicit recovery and background Delete are implemented, and the deleted Workspace's platform refund now requires a fresh identity-bound Fabric readback of the same Delete operation before Control Plane dispatches it. Focused business, restart, concurrency and browser tests plus `verify:local:full` pass with zero required PostgreSQL skips. [Owner evidence](./status.md#retained-runtime-evidence) retains the isolated source and tests. Instance must adopt the same bytes and qualify actual Tencent/TKE and Gateway behavior, including real deletion readback. |
...
| --- | --- | --- | --- | --- | --- |
| A / `PB-A-FABRIC-DELETE-01` | `cloud_complete` | `P0` | Fabric now converges standalone Gateway Secrets, partial PV/PVC bindings, the labelled Runtime tree (controller plus orphaned ReplicaSet/Pod), the CBS deletion and the TKE Machine/CVM retirement through exact persisted ownership and live label discovery, including restart and retained success readback; a surviving owned object, a still-attached or unconfirmed CBS, and a still-terminating CVM keep the deletion retryable instead of terminal. A completion audit's evidence findings are fixed: one observation per stage including waiting and failure, a required readback reference on every provider and ledger observation, an identity-bound receipt digest, and accumulating read and mutation counters | Fabric adapter tests inject each single residue, prove exact-owner deletion and final `absent`, prove the Deployment/ReplicaSet/Pod tree is deleted with waiting mutations and that ambiguous ownership mutates nothing, and reject unknown/conflict without unrelated mutation. Storage deletion tests prove the attached-disk refusal, exactly one terminate after detach, no re-send after an uncertain terminate, readback-only convergence when already absent, and a hard failure for an unverifiable provider response. Compute deletion tests prove typed Machine/CVM facts, a still-terminating CVM and a present Machine as retryable waits, exactly one node deletion with scale-down and terminate mode, readback-only convergence, and fail-closed identity drift and unverifiable readbacks | Same-Candidate Tencent/TKE protected deletion readback |
...
| C / `PB-C-LIFECYCLE-RECONCILERS-01` | `cloud_complete` | `P0` | A completion audit's refund findings are fixed: the refund basis is the confirmed charge that paid for the period in use (an in-force renewal over the original purchase) with per-order bounds, frozen money facts across retries, coverage of resource-only purchases, and an identity-bound live readback that refuses a readback omitting the CBS/machine/CVM identity. Control Plane now owns explicit original-period recovery, Runtime suspension/readback, service-authorized background Delete, the gated platform refund for that Delete, the persisted per-stage deletion evidence the deletion receipt carries, and the stage/reason/last-readback/retry/refund projection that both the customer deletion page and the admin Runtime detail render without a manual completion control; focused/full local regression is complete; actual deployed qualification remains external | Separate Renewal and Delete tests prove claim/lease/CAS, restart recovery, one debit/provider mutation/Receipt, full Delete absence, and explicit manual review | Instance runs the same Candidate through bounded Renewal and Delete readback |
...
| 单用例部署（镜像＋规格） | 部署命令直接接受镜像与运行规格，不再要求先单独登记一次全局应用版本；revision 快照仍只有唯一 owner。 | 2026-09-17 Cloud 侧完成：`POST /api/operator/application-deployments` 接受可选内联 `revision`，在同一命令内写入既有准入表并复用同一部署链；冲突内容拒绝且不改写已存 revision，信封身份不一致拒绝。2026-09-18 完成 Console 侧收敛：登记面板与动作移除（`admitRevision` 及其 API 函数已无调用者，按死代码清理），面板成为 选择镜像 → 填写该镜像真正需要的运行描述 → 部署；操作者不再填写 applicationId/version——命令未命名身份时由 Control Plane 依据镜像 digest 派生稳定内部身份（同一镜像恒等、不同镜像不冲突、无 digest 拒绝而非编造）；简单表单不再预填 8080/`/healthz`/`/data`，不发布入口的策略可显式留空，需要发布入口的策略必须声明真实端口，否则契约 owner 与 Console 都拒绝。取证见 [单用例部署命令](status.md#unified-deployment-command-with-inline-revision-local-development)与 status 的 [统一应用部署命令](status.md#unified-application-deployment-command)、[最小部署描述](status.md#minimal-deployment-description)。剩余：Instance 无部署读回。 |
...
| 应用安装身份与凭据 | 部署命令的镜像更新不得改变同一应用的数据目录与入口；运行时要创建、挂载并暴露 revision 声明的凭据与 secret input，不得为镜像虚构凭据或挂载未创建的文件。 | 2026-09-18 复核修正：应用身份稳定于 Workspace 的应用槽位，版本由描述内容（镜像 digest 与运行事实）派生，因此同应用换镜像只产生新的不可变版本、数据命名空间与入口不变，而同镜像不同合法描述不再冲突；运行时的凭据创建与挂载改为同一谓词（revision 声明什么就创建并挂载什么），并补齐集成 fixture 自身未声明的凭据与 secret input——这才是 `OPL_FABRIC_LOCAL_DOCKER_INTEGRATION` 凭据文件缺失的真实原因，不是目录未共享。取证见 [完整验收修正](status.md#completion-audit-corrections)。 | Instance 采用同一 Candidate 后复核真实部署与凭据读回 |
