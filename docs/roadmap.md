...
| J / `PB-J-OPERATIONS-RECOVERY-01` | `active` | `P0` | Control Plane Operations and Recovery; Launch/Renewal review exists, exact ready Tencent/TKE Runtime Launch review now converges through a system-owned read-only authorization, and disposable reset preview is read-only, while Registration/Delete/Reset recovery and unified closure evidence are incomplete | Every supported manual-review operation appears once with owner, blocker and allowed action; recovery is idempotent, owner-read, audited, and cannot write another owner's state | Protected Instance operator executes only Cloud-authorized commands and retains receipts |
