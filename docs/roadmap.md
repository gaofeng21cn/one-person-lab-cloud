...
| F / `PB-F-TYPED-OPERATIONS-01` | `active` | `P1` | Each Control Plane/Fabric bounded context owns its reducer; live changed paths still contain generic maps/strings and retired Acceptance B surfaces. Fabric callers use capability-specific persistence/provider ports; Workspace Launch Stages and non-secret Runtime status reads have dependency-narrow Engine owners. Control Plane `WorkspaceLaunchReconciler` types Stage, Launch status, and Stage observation state behind its strict schema-v3 codec. Console Workspace Launch, access Secrets, Delete, Renewal, Gateway Budget, Gateway Usage, Billing/Receipt, Wallet Adjustment, Support Mapping, and Operator Account now own complete browser lifecycles behind typed controllers; the broad root retains Session/Router, shared reads, route composition, and unrelated operator commands. Announcement Lifecycle, route read loading, and ConsoleShell state remain open | Select the next complete capability from the inventory, define its narrow typed contract, switch every real consumer, and delete that capability's root state, intent, timer/generation, readback, reset and command. Operator Account is complete with normalized provision intent, per-account claims, command identity/target validation, paged owner readback, response-loss key reuse, late-result rejection, route exit, and Session reset tests; Gateway Usage and Billing/Receipt retain their focused query freshness evidence. Do not move Session until capability reset contracts are explicit | Instance readback must prove no configured or non-terminal Acceptance B consumer before Cloud removes that path |
