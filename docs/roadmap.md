...
| F / `PB-F-TYPED-OPERATIONS-01` | `active` | `P1` | Each Control Plane/Fabric bounded context owns its reducer; live changed paths still contain generic maps/strings and retired Acceptance B surfaces. Fabric callers use capability-specific persistence/provider ports; Workspace Launch Stages and non-secret Runtime status reads have dependency-narrow Engine owners. Control Plane `WorkspaceLaunchReconciler` types Stage, Launch status, and Stage observation state behind its strict schema-v3 codec. Console Workspace Launch, access Secrets, Delete, Renewal, Gateway Budget, Gateway Usage, Billing/Receipt, Wallet Adjustment, Support Mapping, Operator Account, and Operator Announcement Lifecycle now own complete browser lifecycles behind typed controllers; the broad root retains Session/Router, shared reads, route composition, customer announcement reads, route read loading, and ConsoleShell state | Select the next complete capability from the inventory, define its narrow typed contract, switch every real consumer, and delete that capability's root state, intent, timer/generation, readback, reset and command. Operator Announcement is complete with normalized create intent, per-announcement claims, command identity/status/schedule validation, owner readback, response-loss key reuse, late-result rejection, route exit/re-entry, and Session reset tests. Do not merge customer read receipts into the operator state machine or move Session until capability reset contracts are explicit | Instance readback must prove no configured or non-terminal Acceptance B consumer before Cloud removes that path |
