...
Deployment is one command. The operator selects a repository and tag, the server
resolves the tag to an immutable digest, and the same request carries the run
description; there is no separate registration business step. Control Plane admits
that description into the existing single revision owner inside the command, so an
identical version is an idempotent replay and different content under the same
identity is refused rather than overwritten, and an already admitted version can
still be deployed by naming it.

The application identity is not an operator input. When the command omits
`applicationId`/`version`, Control Plane derives the two halves separately: the
application identity is stable for the Workspace's application slot, and the
version is derived from the description's content — its image digest and every
declared run fact. Updating an application's image is therefore a new immutable
version of the same application, so its data namespace and published entry do not
move, while a different legal run description of the same image is a new version
rather than a conflict. An exact replay resolves to the version already admitted,
and a description with no digest, an empty Workspace, or an invalid body is refused
instead of receiving a fabricated identity.

The description carries only the facts the image has. Nothing defaults a port,
health check, persistent mount, dependency or derived credential on the image's
behalf, so a simple image composes a revision with none of them. The runtime
creates, mounts and advertises exactly the credentials and secret inputs a revision
declares, and each platform-issued credential is mounted at the target that
credential declares. Both providers apply that rule: a revision that asks for its
credential elsewhere receives it there, a revision declaring only the Gateway key
receives no WebUI credential file, and a revision that needs only the derived
administrator password receives it without also declaring the Gateway key. The one fact the platform cannot
guess is the entry target: a publishing exposure policy must declare the TCP entry
port, and both the contract owner and the Console refuse a revision that omits it,
while a `cloud_private` revision declares no entry. Fabric refuses to publish an
undeclared entry at runtime as well, so a retained payload cannot bypass the rule.

#### Default Install And Deployment Chain
...
## Workspace Delete And Platform Refund

One original `workspace.delete.v2` operation owns the deletion. It records the
provider identity of the resources it destroyed (runtime, compute, storage,
attachment, the storage provider resource and the compute machine/CVM
identities) and the platform-confirmed fulfillment and deletion times before it
may complete.

Fabric Runtime retirement deletes the labelled owned object set itself —
Deployment, ReplicaSet, Pod, Service, NetworkPolicy and the environment Secret —
waiting for each mutation, and retires the Gateway Secret through its verified
digest-bound path. The deletion targets are discovered from the live Workspace
label instead of a guessed name, so an orphaned ReplicaSet or Pod left behind by
a missing controller still converges. Ownership ambiguity fails closed before the
first mutation, and the Runtime only reports `destroyed` after a fresh readback
proves the controller and every child object are gone. A surviving owned object
returns a pending result, which Control Plane records as a retryable wait on the
same operation; only an unavailable readback or an ownership conflict becomes a
reviewable failure.

Storage retirement follows the same discipline. Fabric verifies the retained
PV/PVC owner identity against the exact static manifest, deletes the binding with
a waiting mutation, and requires the binding to be gone before any CBS RPC. The
provider then reconciles the disk to `UNATTACHED` and refuses to terminate an
attached disk; only a disk that provably dispatched no terminate RPC stays
retryable, and a refusal that may already have sent one is never re-sent. Fabric
classifies each storage deletion outcome as `pending_retry` or
`unconfirmed_send` on the returned volume and reports it with a retrying HTTP
status, so Control Plane keeps the same Delete operation pending instead of
recording a terminal failure. An unverifiable or contradictory provider response
carries no classification and stays a hard failure. The deletion records the CBS
provider identity from the Control Plane projection at claim time, so every later
readback and the platform refund bind to the disk this Workspace was launched
with.

Compute retirement follows the same discipline. Fabric validates the retained
NodePool, Machine, CVM and Node identity before any mutation, and retires the node
through the TKE node-management path with `EnableScaleDown=true` and
`InstanceDeleteMode=terminate`, so TKE keeps owning the CVM lifecycle instead of a
direct instance termination. It then requires the Machine and the CVM to be
confirmed gone separately: a Machine that is absent while its CVM is still being
reclaimed is an ordinary convergence wait, not an absence. Every readback reports
the facts it observed (`machinePresent`, `tkeStatus`, `cvmStatus`) plus the shared
deletion classification, so a caller never infers state from a status string.

A deletion stage records the latest observation it made, not only its
confirmation: a stage that is still waiting or that failed is persisted too, so a
stalled deletion is explainable from state, and only the result the frozen contract
requires for that stage counts as a confirmation. A confirmed entry is immutable
and a waiting or failed entry is replaced by the next observation of the same
stage, so the record is append-only where it matters. Every provider or ledger
observation names the readback it came from — Fabric stamps a readback identity on
its storage and compute destroy readbacks — and the read and mutation counters
accumulate separately, so a query never stands in for a destroy. The
`workspace.deleted.v1` digest carries an opaque reference derived from the resource
identity, the readback and the observation time, which keeps the receipt
identity-sensitive without publishing private provider identifiers.
...
purchase, and the period start is the start of the paid period that charge covers.
The refund never exceeds what that order charged minus what it already refunded,
and an unresolved refund keeps the amount and identity key it was created with
rather than recomputing them from a later clock. A refund is authorised only by a
live readback that reports the exact CBS, machine and CVM identity this Delete
operation destroyed; a readback that omits an identity is refused and an expected
identity is never substituted for an observed one.

The deletion classifications (`pending_retry`, `unconfirmed_send`) have one
owner: `packages/contracts/go`. Fabric, which owns the physical deletion, writes
one of them on an unfinished storage or compute deletion; Control Plane reads the
same vocabulary to keep the original Delete operation open and retryable instead
of recording a terminal result. An empty classification means the deletion either
completed or failed unverifiably, so no caller may treat it as retryable, and a
provider may never classify a deletion as unfinished while also claiming
authoritative absence.

Each deletion stage persists the confirmation it accepted in the same owner write
that advances the phase: the stage, the result, the resource identity, the time the
observing owner observed the fact, the readback record it came from, and separate
read and mutation attempts. The evidence is append-only and the owning write path
refuses to drop or rewrite a recorded confirmation, so a later poll can never
replace what the receipt and the refund gate rely on. A local state transition
records that fact — releasing the mount binding and removing the Workspace
projection are recorded as local transitions — and never claims a provider
readback; the physical CBS detach is proven by the storage stage instead. Fabric
stamps its own observation time on the readbacks it returns, so a recorded
`observedAt` is when the fact was observed rather than when the record was
written.

The final `workspace.deleted.v1` receipt carries the persisted evidence as a
bounded, redacted digest list (stage, result, evidence kind, observation time). It
is written only from confirmations already recorded, and a partial list is refused
rather than recorded. Verification rebuilds the payload from the stages the
receipt attests, so the receipt's own stage is recorded afterwards against the
written receipt. Ledger validates the summary's shape and stage completeness
without re-judging provider facts, and the owner-facing projection republishes the
same digests after decoding them through the shared owner. A receipt retained from
before this contract carries no summary, keeps its original shape, and is never
back-filled. The refund precondition still reads Fabric fresh after the deletion:
the receipt is necessary history, never a substitute for that readback.

The deletion page is a projection, not a second state machine. Control Plane
publishes the platform deletion stage (derived by walking the frozen stage order
against the confirmations the operation itself recorded — Runtime and Secret
absence, mount release, storage absence, compute absence, Workspace removal,
deletion receipt — not from the durable phase, which names the stage that just
completed and would therefore mislabel a long wait such as an in-flight compute
termination), the customer-visible page state
(`waiting`/`retrying`/`blocked`/`completed`), the stable block reason, the next
scheduled readback, and the separately reported refund state. Reading that
projection is a pure store read: it contacts no Fabric endpoint and no Tencent
API, because only the owning delete worker performs provider reads on its own
schedule. Console renders
those facts and never derives a stage from the durable phase, never infers that
resources are gone, and offers no manual completion control. A block is
`blocked` only for an enumerated provider identity conflict; every other stop
reason keeps retrying, and an unknown code stays retryable instead of being
presented as a permanent conflict. The refund result is absent until the
deletion completes, so an unfinished deletion never shows a refund.

The deletion receipt is the durable evidence of the deletion, and it is
retrievable by its owner. `workspace.deleted.v1` is recorded through Ledger with
the deletion operation's own identity, and the deletion status publishes that
receipt id. The owner-scoped receipt route projects that receipt to its account
with the six confirmed absences (`runtimeStatus`, `gatewaySecretStatus`,
`attachmentStatus`, `storageStatus`, `computeStatus`, `workspaceStatus`) and the
delete operation it belongs to; a receipt whose recorded absences are incomplete
is refused instead of published with missing evidence. The customer fee list stays
fee-only (the deletion carries no charge), so the deletion receipt is reached
through the id the deletion status publishes rather than through that list.
Another account cannot retrieve it.

Replaying the deletion and the refund is a store-and-Fabric read, not a re-issue:
a restart resumes the original `workspace.delete.v2` operation from its classified
wait and completes it without a second provider mutation, and a deletion that
finished while its provider readback was unavailable leaves the refund blocked
under its own durable record until a later worker pass dispatches exactly one
wallet refund.

Control Plane owns the platform refund precondition and amount; Fabric owns
Tencent mutation and the authoritative readback; Sub2API only executes the
wallet refund; Ledger records the receipts. Control Plane never calls a Tencent
API, and the Tencent merchant refund is never an input to the customer refund.

After the deletion completes, Control Plane evaluates one gate
(`contracts.PlatformRefundDispatchAllowed`) over a fresh, read-only, identity-
bound readback of the same operation: Workspace Runtime and Gateway Secret
absence, no residual Deployment/ReplicaSet/Pod/Service/NetworkPolicy/environment
Secret, the TKE mount binding gone, PVC/PV absent, CBS `NOT_FOUND` after the
destroy path confirmed it was unattached, Machine absent and CVM `NOT_FOUND`.
Fabric answers these as typed facts (`bindingPresent`, `machinePresent`,
`tkeStatus`, `cvmStatus`) through its storage and compute destroy-readback
surfaces; `GET /fabric/compute-allocations/{id}/destroy-status` is a read-only
provider readback that never mutates resources. The gate is fail-closed: an
unavailable, stale, partial, mutated or identity-mismatched readback, a resource
that is merely shut down or suspended, or a missing `workspace.deleted.v1`
Receipt refuses the refund and leaves the refund operation blocked for a later
attempt. Refusal never rewrites the deletion result.

The refund itself is one deterministic `business_refund` wallet adjustment bound
to the original Launch debit, the delete operation and the policy version in
`WorkspaceDeleteRefundIdempotencyKey`; the existing wallet-adjustment
reservation rejects any refund beyond the remaining original charge. The amount
uses the versioned platform policy (`WorkspaceRefundPolicyVersion`, 720 monthly
hours, one-hour billing precision) in USD micros, so used hours are rounded up
and a Workspace used for a full month refunds nothing. A lost refund response is
recovered by reading the original refund operation's Gateway history instead of
dispatching a second time. Deletion success and refund success are separate
states; the deletion status DTO reports both.
